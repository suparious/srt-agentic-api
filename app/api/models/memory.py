...
class AdvancedSearchQuery(BaseModel):
    ...
    context_type: Optional[str] = Field(
        None, description="The type of context to search within"
    )
    metadata_filters: Optional[Dict[str, Any]] = Field(
        None, description="Filters to apply on metadata"
    )
    relevance_threshold: Optional[float] = Field(
        None, ge=0, le=1, description="The minimum relevance score for results (0 to 1)"
    )
...
class MemoryAddRequest(BaseModel):
    ...
    memory_type: MemoryType = Field(
        ..., description="The type of memory (short-term or long-term)"
    )
...
class MemoryAddResponse(BaseModel):
    ...
    memory_id: str = Field(
        ..., description="The unique identifier assigned to the added memory"
    )
    message: str = Field(
...
class MemoryRetrieveRequest(BaseModel):
    agent_id: UUID = Field(
        ..., description="The ID of the agent to retrieve the memory for"
    )
    memory_type: MemoryType = Field(
        ..., description="The type of memory (short-term or long-term)"
    )
...
class MemorySearchRequest(BaseModel):
    agent_id: UUID = Field(
        ..., description="The ID of the agent to search memories for"
    )
    memory_type: MemoryType = Field(
        ..., description="The type of memory to search (short-term or long-term)"
    )
    query: str = Field(..., description="The search query")
    limit: int = Field(
        default=10, ge=1, le=100, description="The maximum number of results to return"
    )
...
class MemorySearchResponse(BaseModel):
    ...
    results: List[MemoryEntry] = Field(
        ..., description="The list of memory entries matching the search query"
    )
    relevance_scores: Optional[List[float]] = Field(
        None, description="The relevance scores for each result, if calculated"
    )
...
class MemoryDeleteRequest(BaseModel):
    agent_id: UUID = Field(
        ..., description="The ID of the agent to delete the memory for"
    )
    memory_type: MemoryType = Field(
        ..., description="The type of memory (short-term or long-term)"
    )
...
class MemoryOperationRequest(BaseModel):
    ...
    data: Optional[Dict[str, Any]] = Field(
        default=None, description="Data required for the operation"
    )
...
class MemoryOperationResponse(BaseModel):
    ...
    message: Optional[str] = Field(
        default=None, description="An optional message about the operation"
    )
