...
@router.post(
    "/search", response_model=MemorySearchResponse, summary="Search memory entries"
)
async def search_memory_endpoint(
    request: MemorySearchRequest, api_key: str = Depends(get_api_key)
):
    """
    ...
    try:
        ...
        memory_logger.error(
            f"Error searching memories for agent {request.agent_id}: {str(e)}"
        )
...
@router.post(
    ...
    summary="Advanced search memory entries",
)
async def advanced_search_memory_endpoint(
    ...
    memory_type: Optional[MemoryType] = Query(
        None, description="Type of memory to search"
    ),
    ...
    max_results: int = Query(
        10, ge=1, description="Maximum number of results to return"
    ),
    ...
    try:
        ...
        metadata_filters_dict = (
            json.loads(metadata_filters) if metadata_filters else None
        )
        ...
        memory_logger.error(
            f"Error performing advanced search for agent {agent_id}: {str(e)}"
        )
        raise HTTPException(status_code=500, detail=str(e))
