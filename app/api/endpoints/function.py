...
async def execute_function_endpoint(
    ...
    try:
        function_logger.info(
            f"Executing function {request.function_name} for agent: {request.agent_id}"
        )
        ...
        function_logger.info(
            f"Function {request.function_name} executed successfully for agent: {request.agent_id}"
        )
        ...
        function_logger.error(
            f"Error executing function {request.function_name} for agent {request.agent_id}: {str(e)}"
        )
        raise HTTPException(
            status_code=500, detail="An error occurred while executing the function"
        )
...
async def get_available_functions_endpoint(
    ...
    try:
        function_logger.info(
            f"Retrieving available functions for agent: {request.agent_id}"
        )
        agent = agent_manager.agents.get(request.agent_id)
        if not agent:
            raise ValueError(f"Agent not found: {request.agent_id}")
        ...
        function_logger.info(
            f"Successfully retrieved available functions for agent: {request.agent_id}"
        )
        ...
        function_logger.error(
            f"Value error in retrieving available functions: {str(ve)}"
        )
        raise HTTPException(status_code=404, detail=str(ve))
    except Exception as e:
        function_logger.error(
            f"Error retrieving available functions for agent {request.agent_id}: {str(e)}"
        )
        raise HTTPException(
            status_code=500,
            detail="An error occurred while retrieving available functions",
        )
...
async def register_function_endpoint(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="An error occurred while registering the function"
        )
    ...
    try:
        ...
        for agent in agent_manager.agents.values():
            if request.updated_function.name in agent.available_functions:
                agent.available_functions[request.updated_function.name] = (
                    request.updated_function
                )
        ...
        raise HTTPException(
            status_code=500, detail="An error occurred while updating the function"
        )
...
async def assign_function_endpoint(
    ...
    try:
        function_logger.info(
            f"Assigning function {request.function_id} to agent: {request.agent_id}"
        )
        agent = agent_manager.agents.get(request.agent_id)
        if not agent:
            raise ValueError(f"Agent not found: {request.agent_id}")
        function = next(
            (
                f
                for f in agent.available_functions.values()
                if str(f.id) == request.function_id
            ),
            None,
        )
        if not function:
        ...
        function_logger.info(
            f"Successfully assigned function {request.function_id} to agent: {request.agent_id}"
        )
        ...
        raise HTTPException(
            status_code=500, detail="An error occurred while assigning the function"
        )
...
async def remove_function_endpoint(
    ...
    try:
        ...
        function_name = next(
            (
                name
                for name, func in agent.available_functions.items()
                if str(func.id) == function_id
            ),
            None,
        )
        ...
        function_logger.info(
            f"Successfully removed function {function_id} from agent: {agent_id}"
        )
        return FunctionAssignmentResponse(
            agent_id=agent_id,
        ...
        raise HTTPException(
            status_code=500, detail="An error occurred while removing the function"
        )
